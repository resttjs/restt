...
import { error, queryParams, escapeRegexURL } from './utils';
...
export class Resource {
  ...
  urlRegex() {
    ...
    // Return the escaped url regex
    return escapeRegexURL(this.url);
  }
  ...
  match(req) {
    ...
    // Check whether the request method matches the resource method (or is an options request)
    const method = (this.method.toUpperCase() == req.method.toUpperCase() || req.method.toUpperCase() == 'OPTIONS');
  ...
  async resolve(req) {
    ...
    // Create an empty object of fields
    let fields = {};
    ...
    try {
      ...
      // If there is a request body then fields then parse the fields
      if (body.length > 0) fields = JSON.parse(body);
  ...
  // Send back the OPTIONS for preflight
  async preflight() {
    return { ...this.headers, ...response.headers };
  }
