...
export class Resource {
  ...
  async resolve(req) {
    ...
    // Parse the field values from the request body
    const fields = (req.body) ? await req.json() : {};
    ...
    if (requestHeaders['user-agent'].indexOf('curl') > -1) response.headers['curl'] = true;
