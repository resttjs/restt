...
export class ResponseHandler {
  ...
  async render(response) {
    ...
    // Extract the restt version specifications
    const versions = configuration['restt-specs'].versions;

    // Extract the framework label
    const framework = (versions.restt) ? `Restt: ${versions.restt} / Restt-CLI: ${versions.cli}` : `Restt-CLI: ${versions.cli}`;
